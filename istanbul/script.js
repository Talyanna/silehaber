...
async function getDutyPharmacies() {
    ...
    const url =
        "https://eczaneadresi.com/api/public/v1/duty-pharmacies?city=istanbul&limit=200";
    ...
    try {
        ...
        setText(
            ...
            pharmacies.length
                ? `Bugün İstanbul genelinde ${pharmacies.length} nöbetçi eczane listeleniyor.`
...
async function getEarthquakes() {
    ...
    const url =
        ...
        `?start=${start}` +
        `&end=${end}` +
        `&lat=${ISTANBUL.latitude}` +
        `&lon=${ISTANBUL.longitude}` +
        `&maxrad=3` +
        `&limit=3` +
        `&orderby=time` +
        `&format=json`;
...
document.addEventListener(
    ...
    () => {
        ...
        /*
           10 dakikada bir hava,
           deprem ve eczane bilgisini
