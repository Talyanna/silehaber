...
async function getWeather() {
    ...
    const url =
        ...
        `?latitude=${ANTALYA.latitude}` +
        `&longitude=${ANTALYA.longitude}` +
...
async function getDutyPharmacies() {
    ...
    list.innerHTML = `
        ...
        <div class="loading-card">
            ...
            Antalya nöbetçi eczaneleri yükleniyor...
    ...
    const url =
        "https://eczaneadresi.com/api/public/v1/duty-pharmacies?city=antalya&limit=200";
    ...
    try {
        ...
        setText(
            ...
            pharmacies.length
                ? `Bugün Antalya genelinde ${pharmacies.length} nöbetçi eczane listeleniyor.`
        ...
        setText(
            "pharmacy-panel-description",
            "Antalya nöbetçi eczane verisine şu anda ulaşılamıyor."
        );
...
async function getEarthquakes() {
    ...
    const url =
        ...
        `?start=${start}` +
        `&end=${end}` +
        `&lat=${ANTALYA.latitude}` +
        `&lon=${ANTALYA.longitude}` +
        `&maxrad=3` +
        `&limit=3` +
        `&orderby=time` +
        `&format=json`;
